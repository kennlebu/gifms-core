// script.aculo.us effects.js v1.9.0, Thu Dec 23 16:54:48 -0500 2010 Copyright
// (c) 2005-2010 Thomas Fuchs (https://script.aculo.us, https://mir.aculo.us)
// Contributors:  Justin Palmer (https://encytemedia.com/)  Mark Pilgrim
// (https://diveintomark.org/)  Martin Bialasinki
//
// script.aculo.us is freely distributable under the terms of an MIT-style
// license. For details, see the script.aculo.us web site:
// https://script.aculo.us/ converts rgb() and #xxx to #xxxxxx format, returns
// self (or first argument) if not convertable
String.prototype.parseColor = function () {
    var color = '#';
    if (this.slice(0, 4) == 'rgb(') {
        var cols = this
            .slice(4, this.length - 1)
            .split(',');
        var i = 0;
        do {
...
var Effect = {
    _elementDoesNotExistError: {
        ...
        message: 'The specified DOM element does not exist, but is required for this effect to ope' +
                'rate'
    },
    Transitions: {
        linear: Prototype.K,
            ...
            if (child.nodeType == 3) {
                child
                    ...
                    .each(function (character) {
                        element.insertBefore(new Element('span', {style: tagifyStyle}).update(character == ' '
    ...
    toggle: function (element, effect, options) {
        ...
        return Effect[Effect.PAIRS[effect][element.visible()
                    ? 1
                    : 0]](element, Object.extend({
            queue: {
                position: 'end',
...
Effect.Queues = {
    ...
    get: function (queueName) {
        ...
        return this
            ...
            .get(queueName) || this
            .instances
            .set(queueName, new Effect.ScopedQueue());
    }
};
Effect.Queue = Effect
    .Queues
...
Effect.Tween = Class.create(Effect.Base, {
    initialize: function (object, from, to) {
        object = Object.isString(object)
            ? $(object)
            : object;
        var args = $A(arguments),
            method = args.last(),
            options = args.length == 5
                ? args[3]
                : null;
...
Effect.Highlight = Class.create(Effect.Base, {
    ...
    finish: function () {
        this
            .element
            .setStyle(Object.extend(this.oldStyle, {backgroundColor: this.options.restorecolor}));
    }
});

Effect.ScrollTo = function (element) {
    var options = arguments[1] || {},
        scrollOffsets = document
            .viewport
            .getScrollOffsets(),
...
Effect.Appear = function (element) {
    ...
    var options = Object.extend({
        ...
        beforeSetup: function (effect) {
            effect
                .element
                .setOpacity(effect.options.from)
                .show();
        }
    }, arguments[1] || {});
    return new Effect.Opacity(element, options);
...
Effect.SlideUp = function (element) {
    ...
    var oldInnerBottom = element
        ...
        .getStyle('bottom');
    var elementDimensions = element.getDimensions();
    return new Effect.Scale(element, window.opera
        ? 0
        : 1, Object.extend({
        scaleContent: false,
        scaleX: false,
...
// Bug in opera makes the TD containing this element expand for a instance after
// finish
...
Effect.Shrink = function (element) {
    ...
    return new Effect.Parallel([
        ...
        new Effect.Scale(element, window.opera
            ? 1
            : 0, {
            sync: true,
            transition: options.scaleTransition,
    ...
    initialize: function (element) {
        ...
        if (!Object.isString(options.style)) 
            ...
            if (options.style.include(':')) 
                ...
                this
                    .element
                    .addClassName(options.style);
                this.style = $H(this.element.getStyles());
                this
                    .element
                    .removeClassName(options.style);
                var css = this
                    .element
                    .getStyles();
                this.style = this
        ...
        this.transforms = this
            ...
            .map(function (pair) {
                ...
                return {
                    ...
                    originalValue: unit == 'color'
                        ? parseColor(originalValue)
                        : parseFloat(originalValue || 0),
                    targetValue: unit == 'color'
                        ? parseColor(value)
                        : value,
                    unit: unit
                };
            }.bind(this))
            .reject(function (transform) {
    ...
    update: function (position) {
        ...
        while (i--) 
            style[(transform = this.transforms[i]).style] = transform.unit == 'color'
                ...
                : (transform.originalValue + (transform.targetValue - transform.originalValue) * position).toFixed(3) + (transform.unit === null
                    ? ''
                    : transform.unit);
        this
            .element
...
Element.CSS_PROPERTIES = $w('backgroundColor backgroundPosition borderBottomColor borderBottomStyle borderBot' +
        'tomWidth borderLeftColor borderLeftStyle borderLeftWidth borderRightColor border' +
        'RightStyle borderRightWidth borderSpacing borderTopColor borderTopStyle borderTo' +
        'pWidth bottom clip color fontSize fontWeight height left letterSpacing lineHeigh' +
        't marginBottom marginLeft marginRight marginTop markerOffset maxHeight maxWidth ' +
        'minHeight minWidth opacity outlineColor outlineOffset outlineWidth paddingBottom' +
        ' paddingLeft paddingRight paddingTop right textIndent top width wordSpacing zInd' +
        'ex');
...
if (document.defaultView && document.defaultView.getComputedStyle) {
    Element.getStyles = function (element) {
        ...
        return Element
            .CSS_PROPERTIES
            .inject({}, function (styles, property) {
    ...
    Element.getStyles = function (element) {
        ...
        styles = Element
            .CSS_PROPERTIES
            .inject({}, function (results, property) {
...
$w('fade appear grow shrink fold blindUp blindDown slideUp slideDown pulsate shake p' +
            'uff squish switchOff dropOut').each(function (effect) {
    Effect.Methods[effect] = function (element, options) {
        element = $(element);
        Effect[effect
                .charAt(0)
                .toUpperCase() + effect.substring(1)](element, options);
        return element;
    };
});

$w('getInlineOpacity forceRerendering setContentZoom collectTextNodes collectTextNod' +
            'esIgnoreClass getStyles').each(function (f) {
