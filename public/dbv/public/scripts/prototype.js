...
var Prototype = {
    ...
    Browser: (function () {
        ...
        var isOpera = Object
            .prototype
            .toString
            .call(window.opera) == '[object Opera]';
        return {
...
(function () {
            ...
            case 'object':
                ...
                if (_class === ARRAY_CLASS) {
                    for (var i = 0, length = value.length; i < length; i++) {
                        ...
                        partial.push(typeof str === 'undefined'
                            ? 'null'
                            : str);
                    }
    ...
    extend(Object, {
        extend: extend,
        inspect: inspect,
        toJSON: NATIVE_JSON_STRINGIFY_SUPPORT
            ? stringify
            : toJSON,
...
(function (proto) {
    ...
    function toISOString() {
        return this.getUTCFullYear() + '-' + (this.getUTCMonth() + 1).toPaddedString(2) + '-' + this
            .getUTCDate()
            .toPaddedString(2) + 'T' + this
            .getUTCHours()
            .toPaddedString(2) + ':' + this
            .getUTCMinutes()
            .toPaddedString(2) + ':' + this
            .getUTCSeconds()
            .toPaddedString(2) + 'Z';
...
Object.extend(String.prototype, (function () {
    var NATIVE_JSON_PARSE_SUPPORT = window.JSON && typeof JSON.parse === 'function' && JSON
        .parse('{"test": true}')
        .test;
    ...
    return {
        ...
        isJSON: isJSON,
        evalJSON: NATIVE_JSON_PARSE_SUPPORT
            ? parseJSON
            : evalJSON,
        include: include,
...
var Template = Class.create({
    ...
    evaluate: function (object) {
        ...
        return this
            .template
            .gsub(this.pattern, function (match) {
                if (object == null) 
                    return (match[1] + '');
...
var Enumerable = (function () {
    ...
    function inGroupsOf(number, fillWith) {
        fillWith = Object.isUndefined(fillWith)
            ? null
            : fillWith;
...
Ajax.Request = Class.create(Ajax.Base, {
    ...
    request: function (url) {
        ...
        try {
            ...
            this
                .transport
                .open(this.method.toUpperCase(), this.url, this.options.asynchronous);

            if (this.options.asynchronous) 
                this.respondToReadyState.bind(this).defer(1);
            
            this.transport.onreadystatechange = this
                .onStateChange
                .bind(this);
            this.setRequestHeaders();

            this.body = this.method == 'post'
                ? (this.options.postBody || params)
                : null;
            this
                .transport
                .send(this.body);
    ...
    respondToReadyState: function (readyState) {
        ...
        if (state == 'Complete') {
            try {
                this._complete = true;
                (this.options['on' + response.status] || this.options['on' + (this.success()
                        ? 'Success'
                        : 'Failure')] || Prototype.emptyFunction)(response, response.headerJSON);
            } catch (e) {
                this.dispatchException(e);
            }
        ...
        try {
            ...
            Ajax
                .Responders
                .dispatch('on' + state, this, response, response.headerJSON);
        } catch (e) {
            this.dispatchException(e);
        }
...
Ajax.Response = Class.create({
    initialize: function (request) {
        ...
        if (readyState == 4) {
            ...
            this.responseXML = Object.isUndefined(xml)
                ? null
                : xml;
    ...
    _getResponseJSON: function () {
        ...
        try {
            return this
                .responseText
                .evalJSON(options.sanitizeJSON || !this.request.isSameOrigin());
        } catch (e) {
            this
                .request
                .dispatchException(e);
...
Ajax.Updater = Class.create(Ajax.Request, {
    initialize: function ($super, container, url, options) {
        this.container = {
            ...
            failure: (container.failure || (container.success
                ? null
                : container))
        };
    ...
    updateContent: function (responseText) {
        var receiver = this.container[this.success()
                    ? 'success'
                    : 'failure'],
...
Element.Methods = {
        ...
        function update(element, content) {
            ...
            if (ANY_INNERHTML_BUGGY) {
                if (tagName in Element._insertionTranslations.tags) {
                    ...
                    Element
                        ._getContentFromAnonymousElement(tagName, content.stripScripts())
                        .each(function (node) {
    ...
    up: function (element, expression, index) {
        ...
        return Object.isNumber(expression)
            ...
            : Prototype
                .Selector
                .find(ancestors, expression, index);
    },
    ...
    previous: function (element, expression, index) {
        ...
        if (expression) {
            return Prototype
                .Selector
                .find(element.previousSiblings(), expression, index);
        } else {
    ...
    next: function (element, expression, index) {
        ...
        if (expression) {
            return Prototype
                .Selector
                .find(element.nextSiblings(), expression, index);
        } else {
            var maximumLength = Object.isNumber(index)
                ? index + 1
                : 1;
    ...
    adjacent: function (element) {
        ...
        return Prototype
            .Selector
            .select(expressions, element.parentNode)
            .without(element);
    },
    ...
    writeAttribute: function (element, name, value) {
        ...
        if (typeof name == 'object') 
            ...
            attributes[name] = Object.isUndefined(value)
                ? true
                : value;
...
if (Prototype.Browser.Opera) {
    Element.Methods.getStyle = Element
        .Methods
        .getStyle
        .wrap(function (proceed, element, style) {
            switch (style) {
                case 'height':
                case 'width':
    ...
    Element.Methods.readAttribute = Element
        .Methods
        .readAttribute
    ...
    $w('colSpan rowSpan vAlign dateTime accessKey tabIndex encType maxLength readOnly lo' +
                'ngDesc frameBorder').each(function (attr) {
...
(function () {
    ...
    Object.extend(Element.Layout, {
        PROPERTIES: $w('height width top left right bottom border-left border-right border-top border-bo' +
                'ttom padding-left padding-right padding-top padding-bottom margin-top margin-bot' +
                'tom margin-left margin-right padding-box-width padding-box-height border-box-wid' +
                'th border-box-height margin-box-width margin-box-height'),

        COMPOSITE_PROPERTIES: $w('padding-box-width padding-box-height margin-box-width margin-box-height border-b' +
                'ox-width border-box-height'),
...
Prototype.Selector = (function () {
    ...
    return {
        ...
        match: match,
        find: find,
        extendElements: (Element.extend === K)
            ? K
            : extendElements,
...
(function () {
    ...
    try {
        Array
            .prototype
            .slice
            .call(document.documentElement.childNodes, 0);
        ...
        if (!!document.getElementById(id)) {
            Expr.find.ID = function (match, context, isXML) {
                if (typeof context.getElementById !== "undefined" && !isXML) {
                    var m = context.getElementById(match[1]);
                    return m
                        ? m.id === match[1] || typeof m.getAttributeNode !== "undefined" && m
                            .getAttributeNode("id")
                            .nodeValue === match[1]
                            ? [m]
                            : undefined
...
Form.Element.Serializers = (function () {
    ...
    function selectOne(element) {
        ...
        return index >= 0
            ? optionValue(element.options[index])
            : null;
    }
    ...
    function optionValue(opt) {
        return Element.hasAttribute(opt, 'value')
            ? opt.value
            : opt.text;
    }
...
(function () {
    ...
    if (window.addEventListener) {
        Event.prototype = window.Event.prototype || document
    ...
    function on(element, eventName, selector, callback) {
        ...
        return new Event
            .Handler(element, eventName, selector, callback)
            .start();
    }
...
(function () {
    window.Selector = Class.create({
        ...
        findElements: function (rootElement) {
            return Prototype
                .Selector
                .select(this.expression, rootElement);
        },

        match: function (element) {
            return Prototype
                .Selector
                .match(element, this.expression);
        },
